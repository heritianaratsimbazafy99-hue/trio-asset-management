...
export function getWorkflowRequestTypeLabel(type) {
  ...
  if (normalized === WORKFLOW_REQUEST_TYPES.ASSET_PURCHASE_VALUE_CHANGE) {
    return "Changement de valeur d'achat";
  }
  if (normalized === WORKFLOW_REQUEST_TYPES.MAINTENANCE_START) {
    return "Validation de maintenance";
  }
...
export function getWorkflowStepLabel(request) {
  const type = String(request?.request_type || "").toUpperCase();
  const status = String(request?.status || "").toUpperCase();
  const approvalCount = Number(request?.approval_count || 0);
  const requiredApprovals = Number(request?.required_approvals || 0);

  if (type !== WORKFLOW_REQUEST_TYPES.MAINTENANCE_START) {
    if (requiredApprovals > 1) {
      return `Progression ${approvalCount} / ${requiredApprovals}`;
    }
    return "Validation simple";
  }

  if (status === WORKFLOW_REQUEST_STATUS.APPROVED) {
    return "Maintenance autorisée par le CEO";
  }

  if (status === WORKFLOW_REQUEST_STATUS.REJECTED) {
    return "Ticket maintenance rejeté";
  }

  if (status === WORKFLOW_REQUEST_STATUS.FAILED) {
    return "Ticket maintenance en échec";
  }

  if (approvalCount <= 0) {
    return "Étape 1/2: conformité DAF";
  }

  return "Étape 2/2: accord CEO pour démarrage";
}

export function getWorkflowApproveActionLabel(request, userRole) {
  const type = String(request?.request_type || "").toUpperCase();
  ...
  if (approvalCount <= 0) {
    return "Valider conformité";
  }

  if (approvalCount >= 1) {
    return "Autoriser démarrage";
  }

  if (role === APP_ROLES.DAF) {
    return "Valider conformité";
  }
...
export function getWorkflowRejectActionLabel(request, userRole) {
  const type = String(request?.request_type || "").toUpperCase();
  const role = String(userRole || "").toUpperCase();
  const approvalCount = Number(request?.approval_count || 0);
  ...
  if (approvalCount <= 0) {
    return "Rejeter conformité";
  }

  if (approvalCount >= 1) {
    return "Refuser démarrage";
  }
