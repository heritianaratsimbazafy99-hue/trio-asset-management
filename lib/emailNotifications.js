...
function normalizeNotificationPayload(queueItem) {
  ...
  return {
    ...
    customEmailSubject: String(payload?.email_subject || "").trim(),
    customTitle: String(payload?.title || "").trim(),
    customBody: String(payload?.body || "").trim(),
    customCtaLabel: String(payload?.cta_label || "").trim(),
  };
}
...
function looksLikeTechnicalError(value) {
  return /column reference|sqlstate|violates|invalid input syntax|duplicate key|permission denied|relation .* does not exist|function .* does not exist/i.test(
    String(value || "")
  );
}

function getSafeWorkflowFailureCopy(value) {
  const rawValue = String(value || "").trim();
  if (!rawValue) return "";
  if (!looksLikeTechnicalError(rawValue)) return rawValue;
  return "Incident technique enregistre. Consultez les validations dans Trio Asset pour le detail.";
}
...
function buildEmailScenario(queueItem, baseUrl) {
  ...
  if (normalizedType === "INCIDENT_ALERT") {
    ...
    const defaultSubject = `Alerte incident${assetName ? ` - ${assetName}` : ""}`;
    const defaultTitle = incidentTitle || fallbackTitle;
    const defaultBody = assetName
    ...
    return {
      ...
      actionLabel: customCtaLabel || (assetName ? "Ouvrir l'actif" : "Ouvrir l'incident"),
  ...
  if (normalizedType === "WORKFLOW_PENDING") {
    ...
    const defaultSubject = `Validation requise - ${requestTypeLabel}${assetName ? ` - ${assetName}` : ""}`;
    const defaultTitle = `Validation requise - ${requestTypeLabel}`;
    const defaultBody = `Une demande de ${requestTypeLabel} attend votre decision.`;
    return {
      subject: customEmailSubject || defaultSubject,
      title: customTitle || defaultTitle,
      body: customBody || defaultBody,
      actionLabel: customCtaLabel || "Traiter la demande",
      notificationTypeLabel,
      recipientLabel,
      actionUrl,
      accent: "#d97706",
  ...
  if (["WORKFLOW_APPROVED", "WORKFLOW_REJECTED", "WORKFLOW_FAILED"].includes(normalizedType)) {
    const safeCustomBody =
      normalizedType === "WORKFLOW_FAILED" && looksLikeTechnicalError(customBody)
        ? ""
        : customBody;
    const safeResolutionNote =
      normalizedType === "WORKFLOW_FAILED"
        ? getSafeWorkflowFailureCopy(resolutionNote)
        : resolutionNote || fallbackBody;
    const statusCopy =
    ...
    return {
      subject:
        customEmailSubject ||
        `${statusCopy.subjectPrefix} - ${requestTypeLabel}${assetName ? ` - ${assetName}` : ""}`,
      title: customTitle || `${statusCopy.title} - ${requestTypeLabel}`,
      body: safeCustomBody || statusCopy.body,
      actionLabel: customCtaLabel || "Ouvrir dans Trio Asset",
      notificationTypeLabel,
      recipientLabel,
      actionUrl,
      ...
      details: compactDetails([
        ...
        { label: "Note de resolution", value: safeResolutionNote },
      ]),
    };
  }

  return {
    subject: customEmailSubject || String(queueItem?.subject || payload?.title || "Notification Trio Asset"),
    title: customTitle || fallbackTitle,
    body: customBody || fallbackBody,
    actionLabel: customCtaLabel || "Ouvrir dans Trio Asset",
    notificationTypeLabel,
    recipientLabel,
    actionUrl,
